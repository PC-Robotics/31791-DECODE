...
@Autonomous(name = "Far Shot No Movement")
public class FarNoMove extends LinearOpMode {
        ...
        while (!opModeIsActive()) {
            ...
            if(Blue.getTagID() == 24) {
                if (Bearing > -3 && Bearing < 1 && Dist > 94 && Dist < 108) {
                    Blue.setRGBColor(AprilTagVision.RGBColor.RED); // RED

                } else {
                    Blue.setRGBColor(AprilTagVision.RGBColor.VIOLET);

                }
            }
            if(Blue.getTagID() == 20){
                if (Bearing > -7 && Bearing < -3 && Dist > 94 && Dist < 108) {
                    Blue.setRGBColor(AprilTagVision.RGBColor.BLUE); // RED
